...
export default function StoryEditForm({
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        setIsSaving(true)
        try {
            const saved = await updateStory(title, content)
            // fetch new histories or just prepend, but simpler to rely on server action
            // For now, let's just alert
            alert('保存しました')

            // Update local histories with new one at the top
            setHistories([{
                id: 'temp-' + Date.now(),
                title,
                content,
                            ...
                            {histories.length === 0 ? (
                                ...
                                histories.map((h, i) => (
                                    <div key={h.id} className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl flex flex-col gap-2">
                                        ...
                                        <div className="text-sm line-clamp-2 text-slate-600 dark:text-slate-400">{h.content}</div>
                                    </div>
                                ))
                            )}
                        </div>
